...
function get_bazel() {
  if [[ $(uname) == "Darwin" ]]; then
    ...
    curl https://github.com/bazelbuild/bazel/releases/download/4.2.1/bazel-4.2.1-darwin-x86_64  -Lo tools/bazel --retry 4 --retry-delay 30 --retry-all-errors
    # verify content
    echo '74d93848f0c9d592e341e48341c53c87e3cb304a54a2a1ee9cff3df422f0b23c  tools/bazel' | shasum -a 256 -c >/dev/null
  else
    # download bazel version
    curl https://ossci-linux.s3.amazonaws.com/bazel-4.2.1-linux-x86_64 -o tools/bazel --retry 4 --retry-delay 30 --retry-all-errors
    # verify content
