...
for model in sorted(listdir(KERNEL_DIR)):
    ...
    if model < "amp_training_RobertaForCausalLM":
        continue
    ...
    for kernel in sorted(listdir(model_path)):
        ...
        for py in listdir(kernel_path):
            ...
            cmd = """CUDA_VISIBLE_DEVICES=0 TORCHINDUCTOR_MAX_AUTOTUNE_POINTWISE=1 TORCHINDUCTOR_COORDINATE_DESCENT_TUNING=1 TORCHINDUCTOR_CACHE_DIR=[[CACHE_DIR]] TORCH_LOGS="+inductor" TORCHINDUCTOR_BENCHMARK_KERNEL=1 TORCHINDUCTOR_COORDINATE_DESCENT_RADIUS=2 TORCHINDUCTOR_COORDINATE_DESCENT_CHECK_ALL_DIRECTIONS=1 python3 [[PY_PATH]] > [[LOG_PATH]] 2>&1"""
            cmd = (
