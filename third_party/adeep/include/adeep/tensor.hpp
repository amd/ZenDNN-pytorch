...
#ifndef ADEEP_TENSOR_HPP
#define ADEEP_TENSOR_HPP
...
class tensor : public memory {
 public:
  ...
  struct desc : public memory::desc {
    ...
    desc(const zendnn_memory_desc_t &adata)
        : memory::desc(adata) {};
    ...
    inline dims get_strides() const {
      ADEEP_ENFORCE(is_plain(), "Call to_public() before get_strides()");
    ...
    desc permute(const std::vector<int> &permute_axes = {}) const {
      ...
      if (perms.empty()) {
        ...
        ADEEP_ENFORCE(perms.size() == data.ndims,
        ...
        for (auto i = 0; i < perms_sorted.size(); ++i) {
          ADEEP_ENFORCE(perms_sorted[i] == i,
    ...
    desc to_dims(const dims &adims) const {
      ADEEP_ENFORCE(adims.size() == data.ndims, "Rank mismatched.");
    ...
    const blocking_desc_t &blocking_desc() const {
      ADEEP_ENFORCE(is_blocking_desc(),
   ...
   private:
    ...
    bool is_blocking_desc() const { return format_kind() == zendnn_blocked; }

    bool is_wino_desc() const { return format_kind() == zendnn_format_kind_wino; }
    ...
    dims_t& blocking_strides() const {
      ADEEP_ENFORCE(is_blocking_desc(),
                    "Cannot get blocking desc on a non-blocking desc");
      return const_cast<zendnn_memory_desc_t&>(data).format_desc.blocking.strides;
    }
  ...
  tensor make_grouped_weights(int groups, bool is_deconv = false) const {
    ...
    if (old_groups > 1) {
      ...
      ADEEP_ENFORCE(old_groups == groups,
  ...
  tensor &reshape(const dims &adims) {
    ADEEP_ENFORCE(has_same_volume(adims), "reshape to incompatible shape");
  ...
  inline void reorder_from(const tensor &src) {
    ...
    auto pd = zendnn::reorder::primitive_desc(src, *this, op_attr);

    tensor scratchpad(pd.scratchpad_desc());
    zendnn::reorder(pd).execute(
        stream::default_stream(),
        {{ZENDNN_ARG_FROM, const_cast<tensor&>(src)},
         {ZENDNN_ARG_TO, *this},
  ...
  inline void reorder_to(tensor &dst, const attr_t &aattr = attr_t()) const {
    ...
    op_attr.set_scratchpad_mode(zendnn::scratchpad_mode::user);
    auto pd = zendnn::reorder::primitive_desc(*this, dst, op_attr);

    tensor scratchpad(pd.scratchpad_desc());
    zendnn::reorder(pd).execute(
        stream::default_stream(),
        {{ZENDNN_ARG_FROM, const_cast<tensor&>(*this)},
         {ZENDNN_ARG_TO, dst},
         {ZENDNN_ARG_SCRATCHPAD, scratchpad}});
  }
  ...
  void feed_from(const tensor &src, bool is_deconv_weights = false) {
    ...
    if (has_scale() && src.has_scale()) {
      ...
      dst_scale = ADEEP_DEF_SCALE;
      src_scale = ADEEP_DEF_SCALE;
    }
    ADEEP_ENFORCE(dst_scale.size() == src_scale.size(), "Invalid tensor scales");
  ...
  tensor dequantize() const {
    ...
    ADEEP_ENFORCE(has_scale(), "Can not find scales");
    // TODO(xpz): support per-channel dequantize
    ADEEP_ENFORCE(get_scale().size() == 1, "Incorrect scale size");
    dst.feed_from(*this);
    return dst;
  }
  ...
  void insert_submemory(const tensor &src, const dims &adims,
    ...
    auto pd = zendnn::reorder::primitive_desc(
