...
#ifndef ADEEP_UTILS_CPP
#define ADEEP_UTILS_CPP
...
#ifndef ADEEP_USE_MKL
  ADEEP_ENFORCE(0, "can not use bernoulli_generate without MKL support");
#else
...
inline zendnn::algorithm rnn_kind_to_algorithm(rnn_kind rnn) {
  if (rnn == RNN_RELU || rnn == RNN_TANH) {
    return zendnn::algorithm::vanilla_rnn;
  } else if (rnn == LSTM) {
    return zendnn::algorithm::vanilla_lstm;
  } else if (rnn == GRU) {
    return zendnn::algorithm::lbr_gru;
  } else {
    return zendnn::algorithm::undef;
  }
}

inline zendnn::algorithm rnn_kind_to_activation(rnn_kind rnn) {
  if (rnn == RNN_RELU) {
    return zendnn::algorithm::eltwise_relu;
  } else if (rnn == RNN_TANH || rnn == LSTM || rnn == GRU) {
    return zendnn::algorithm::eltwise_tanh;
  } else {
    return zendnn::algorithm::undef;
  }
}
...
inline int set_verbose(int level) {
  zendnn::status ret = zendnn::set_verbose(level);
  return ret == zendnn::status::success;
}
