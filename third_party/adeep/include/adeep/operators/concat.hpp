...
#ifndef ADEEP_OPERATORS_CONCAT_HPP
#define ADEEP_OPERATORS_CONCAT_HPP

namespace adeep {

struct concat : public zendnn::concat {

  using super = zendnn::concat;
  ...
  static void compute(const std::vector<tensor>& inputs,
    ...
    // ZENDNN currently supports two types of implementations in the concat:
    ...
    exec_args args {{ZENDNN_ARG_DST, output}, {ZENDNN_ARG_SCRATCHPAD, scratchpad}};

    for (int i = 0; i < opt_inputs.size(); ++i) {
      args.insert({ZENDNN_ARG_MULTIPLE_SRC + i, opt_inputs[i]});
    }
  ...
  static std::vector<int32_t> compute(
    ...
    scale_t min_scale(ADEEP_DEF_SCALE);
    if (utils::one_of(dst_data_type, data_type::s8, data_type::u8)) {
      ...
      for (auto i : inputs) {
        if (i.get_data_type() != dst_data_type) {
          min_scale = ADEEP_DEF_SCALE;
          dst_data_type = data_type::f32;
          break;
        }
        if (i.has_scale() && (min_scale[0] > i.get_scale()[0])) {
          ADEEP_ENFORCE(i.get_scale().size() == 1, "incorrect scale size");
    ...
    scale_t scales(1);
    // FIXME: To avoid view issue in zendnn
    // NOTE: In zendnn concat, dim 3 and 6+ are not supported.
