...
#ifndef ADEEP_OPERATORS_BATCHNORM_HPP
#define ADEEP_OPERATORS_BATCHNORM_HPP
#include "sum.hpp"

namespace adeep {

struct batch_normalization_forward_inference
    : public zendnn::batch_normalization_forward {

  using super = zendnn::batch_normalization_forward;
    ...
    if (use_stats) {
      ...
      super(pd).execute(stream::default_stream(),
                        {{ZENDNN_ARG_SRC, expected_src},
                         ...
                         {ZENDNN_ARG_VARIANCE, expected_var},
                         {ZENDNN_ARG_MEAN, expected_mean},
                         {ZENDNN_ARG_DST, dst},
...
struct batch_normalization_forward_training
  ...
  static void compute(const tensor& src,
    ...
    exec_args args {{ZENDNN_ARG_SRC, expected_src},
                    {ZENDNN_ARG_SCALE_SHIFT, scale_shift},
  ...
  static void compute(const tensor& src,
   ...
   adeep::sum::compute({momentum, 1 - momentum}, {running_mean, mean},
                       running_mean);
   adeep::sum::compute({momentum, 1 - momentum}, {running_var, variance},
                       running_var);
  }
};

struct batch_normalization_backward
    : public zendnn::batch_normalization_backward {

  using super = zendnn::batch_normalization_backward;
  ...
  static void compute(const tensor& src,
    ...
    auto src_desc = src._get_unblocked_desc_if_4c_blocked();
    // auto src_desc = src.get_desc();
    auto forward_hints = zendnn::batch_normalization_forward::primitive_desc(
    ...
    exec_args args {{ZENDNN_ARG_SRC, expected_src},
                    ...
                    {ZENDNN_ARG_SCALE_SHIFT, scale}, // only need scale
                    {ZENDNN_ARG_MEAN, expected_mean},
                    {ZENDNN_ARG_VARIANCE, expected_variance},
                    {ZENDNN_ARG_DIFF_SRC, diff_src},
                    {ZENDNN_ARG_DIFF_SCALE_SHIFT, diff_scale_shift},
                    {ZENDNN_ARG_SCRATCHPAD, scratchpad}};
    if (with_workspace) {
      args.insert({ZENDNN_ARG_WORKSPACE, dst.get_workspace()});
    }
