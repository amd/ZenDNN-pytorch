...
void registerLogComponent(std::string component_alias, std::string log_qname) {
  ...
  _component_log_funcs[std::string(component_alias)] = component_log_func;
}
...
PyLogHandler::~PyLogHandler() noexcept(false) {
  ...
  if (!log_buffer.empty()) {
    ...
    for (const auto& log : log_buffer) {
      ...
      auto component_alias = log.component_alias();
      processErrorMsgInplace(msg);

      if (_component_log_funcs.find(component_alias) ==
          _component_log_funcs.end()) {
        ...
        std::stringstream ss;
        ss << "Logger with component alias '" << component_alias
           << "' was not registered";
        PyErr_SetString(PyExc_SystemError, ss.str());

      } else {
        PyObject* log_func = _component_log_funcs[std::string(component_alias)];
        auto args =
