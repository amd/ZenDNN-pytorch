...
def _fuse_conv_bn_qat(m: GraphModule) -> GraphModule:
    ...
    for mr in match_and_replacement:
        ...
        for replacement in mr.replacements:
            if replacement.op == "call_function" and replacement.target == torch.ops.aten.convolution.default:
                replacement_conv_node = replacement
            elif replacement.op == "call_function" and replacement.target == torch.ops.aten._native_batch_norm_legit.default:
                replacement_bn_node = replacement
            elif replacement.op == "call_function" and replacement.target == operator.getitem:
        ...
        for orig_node in mr.nodes_map.values():
            if orig_node.target == torch.ops.aten.convolution.default:
                replacement_conv_node.meta = orig_node.meta
            if orig_node.target == torch.ops.aten._native_batch_norm_legit.default:
                replacement_bn_node.meta = orig_node.meta
            if orig_node.target == operator.getitem:
                replacement_getitem_node.meta = orig_node.meta
