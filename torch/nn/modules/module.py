...
class Module:
    ...
    def register_load_state_dict_post_hook(self, hook):
        r"""
        These hooks will be called arguments consisting of the
        current module this hook is registered on and a
        ``NamedTuple`` consisting of attributes ``missing_keys``
        and ``unexpected_keys``. ``missing_keys`` is a ``list``
        of ``str`` containing the missing keys and ``unexpected_keys``
