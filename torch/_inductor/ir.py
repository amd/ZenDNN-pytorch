...
class CollectiveKernel(ExternKernel):
    ...
    def codegen(self, wrapper):
        ...
        wrapper.writeline(
            f"{output_name}_pg = _find_or_create_pg_by_ranks_and_tag('{tag}', {ranks}, {group_size})", self
        )

        self.codegen_output(wrapper, output_name, input_names)
        self.codegen_collective(wrapper, output_name, input_names)
        wrapper.writeline(f"_register_tensor_work({output_name}, {output_name}_work)", self)
