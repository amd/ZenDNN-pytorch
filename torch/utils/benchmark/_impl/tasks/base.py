...
def parse_f(
    f: typing.Callable  # type: ignore[type-arg]
) -> typing.Tuple[inspect.Signature, str]:
    ...
    for node in f_ast.body[0].body:
        ...
        if node.col_offset == -1:
            if not isinstance(getattr(node, "value", None), ast.Str):
                raise ValueError(f"Expected `ast.Str`, got {type(node)}. ({node}) {node.lineno}")
            continue
    ...
    def outer(f: typing.Callable[..., typing.Any]) -> typing.Callable[..., typing.Any]:
        ...
        def inner(
            ...
            **kwargs: typing.Any
            ...
            src = "\n".join([
                "def __run_in_worker_f():",
                textwrap.indent("\n".join(body), " " * 4),
                textwrap.dedent("""
                try:
                    # Clear prior value if it exists.
                    del __run_in_worker_result

                except NameError:
                    pass

                __run_in_worker_result = __run_in_worker_f()
                """)
            ])
            ...
            if has_return_value:
                self.worker.run(src)
                return self.worker.load("__run_in_worker_result")

            else:
                src = "\n".join([src, "assert __run_in_worker_result is None"])
                self.worker.run(src)
