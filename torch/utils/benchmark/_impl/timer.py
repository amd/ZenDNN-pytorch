...
class Timer:
    ...
    Extensions of the above arguments:

        `teardown`, `global_setup`
    ...
    Args:
        ...
        teardown: Optional code to be run at the end of a measurement.
    ...
    def __init__(
        ...
        *,
        ...
        num_threads: int = 1,
        language: Union[specification.Language, specification.LANGUAGE_STR_LITERALS] = specification.Language.PYTHON,
    ):
        self._work_spec = specification.WorkSpec(
            stmt=stmt,
            setup=setup,
            teardown=teardown,
        ...
        self._metadata: specification.WorkMetadata = specification.WorkMetadata(
            label=label,
            sub_label=sub_label,
        ...
        if self._work_spec.language == specification.Language.CPP:
        ...
        if self._work_spec.language == specification.Language.PYTHON:
    ...
    def collect_callgrind(self, number: int = 100) -> None:
        ...
        out_fpath = self._callgrind_task.collect(n_iter=number)
        annotated_inclusive = callgrind_worker.CallgrindWorker.annotate(out_fpath, inclusive=True)
        annotated_exclusive = callgrind_worker.CallgrindWorker.annotate(out_fpath, inclusive=False)
        print(annotated_inclusive)
        print(annotated_exclusive)
