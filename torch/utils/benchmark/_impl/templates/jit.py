...
class TemplateCompiler:
    ...
    _singleton: typing.Optional["TemplateCompiler"] = None

    @classmethod
    def singleton(cls) -> "TemplateCompiler":
        with cls._lock:
            if cls._singleton is None:
                cls._singleton = TemplateCompiler()
    ...
    def __init__(self) -> None:
        self._py_template: str = inspect.getsource(py_template).replace(
            ...
            f"class {specification.COMPILED_MODULE_NAME}:",
        )
    ...
    @staticmethod
    def replace(template: str, work_spec: specification.WorkSpec) -> str:
        pattern_map = {
            "STMT_TEMPLATE_LOCATION": work_spec.stmt,
            "SETUP_TEMPLATE_LOCATION": work_spec.setup,
            "TEARDOWN_TEMPLATE_LOCATION": work_spec.teardown,
            "GLOBAL_SETUP_TEMPLATE_LOCATION": work_spec.global_setup,
        }
    ...
    def apply_template(self, work_spec: specification.WorkSpec) -> str:
        if work_spec.language == specification.Language.PYTHON:
            return self.replace(
                template=self._py_template,
                work_spec=work_spec,
            )

        assert work_spec.language == specification.Language.CPP
        source = self.replace(
        ...
        return textwrap.dedent(f"""
            ...
            {specification.COMPILED_MODULE_NAME} = _jit_cpp_template()
        """)


def generate(work_spec: specification.WorkSpec) -> str:
    return TemplateCompiler.singleton().apply_template(work_spec)
...
def get() -> CompiledTemplate:
    ...
    compiled_module = calling_frame.f_globals.get(specification.COMPILED_MODULE_NAME, None)
    if compiled_module is None:
        raise ValueError(
            f"{specification.COMPILED_MODULE_NAME} was not defined. "
