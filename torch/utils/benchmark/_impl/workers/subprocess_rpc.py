...
if IS_WINDOWS:
    ...
    def to_handle(fd: typing.Optional[int]) -> typing.Optional[int]:
        return None if fd is None else msvcrt.get_osfhandle(fd)  # type: ignore[attr-defined]

    def from_handle(handle: typing.Optional[int], mode: int) -> typing.Optional[int]:
        return None if handle is None else msvcrt.open_osfhandle(handle, mode)  # type: ignore[attr-defined]

else:
    to_handle = lambda fd: fd  # noqa: E731
    from_handle = lambda fd, _: fd  # noqa: E731
...
class _TimeoutPIPE:
    ...
    def _loop(self) -> None:
        while True:
            ...
            # Spawning a thread is expensive, so as long as we're reading and
            # writing to Pipes we want to keep this loop alive. However, it is
            # poor form to contaminate a process with a perpetual thread so
            # once we have gone a fair amount of time without doing any work
            # the thread voluntarily exits. (`_push` will spawn a new thread
            # if there are subsequent calls.)
            if (
                not self._active_reads
                and (now - self._last_interaction) > self._inactivity_threshold
            ):
                self._thread = None
                return
    ...
    def _push(self, w_fd: int, timeout: float) -> None:
        with self._loop_lock:
            ...
            self._active_reads[w_fd] = (timeout, time.time())

    def _pop(self, w_fd: int) -> None:
        with self._loop_lock:
    ...
    def maybe_timeout_read(cls, pipe: "Pipe") -> typing.Generator[None, None, None]:
        timeout = pipe.timeout

        # Timeouts are only set by the parent process. Workers (subprocess)
        # will never set a timeout. (And so we simply no-op.)
        if timeout is None:
            yield
            return
...
class Pipe:
    ...
    read_fd: typing.Optional[int]
    write_fd: typing.Optional[int]
    ...
    def __init__(
        ...
        timeout_callback: typing.Callable[[], None] = (lambda: None),
    ) -> None:
    ...
    def _read(self, size: int) -> bytes:
        ...
        if check_bytes != _CHECK:
            raise IOError(f"{check_bytes!r} != {_CHECK!r}, {msg!r}")
...
class SerializedException:
    ...
    @staticmethod
    def from_exception(e: BaseException, tb: types.TracebackType) -> "SerializedException":
