...
def _unshard(
    ...
    with state._device_handle.stream(unshard_stream):
        # print("GOING TO UNSHARD?", handles)
        for handle in handles:
            handle.unshard()
            handle.post_unshard()
...
def _pre_forward_unshard(
    ...
    if not state._handles_prefetched.get(handles_key, False):
        _unshard(
            state, handles, state._streams_unshard, state._streams_pre_unshard
        )
...
def _post_backward_hook(
    ...
    *unused: Any,
    ...
    with torch.autograd.profiler.record_function(
        ...
        state._streams_post_backward.wait_stream(
            state._device_handle.current_stream()
        )
...
def _prefetch_handles(
    ...
    for handles_key in handles_to_prefetch:
        ...
        _unshard(
            state, handles_key, state._streams_unshard, state._streams_pre_unshard
        )
