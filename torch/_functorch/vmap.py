...
def unwrap_batched(pytree, vmap_level):
    leafs, spec = tree_flatten(pytree)
    something = [unwrap_batched_leaf(leaf, vmap_level) for leaf in leafs]
    leafs, bdims = zip(*something)
    return tree_unflatten(leafs, spec), tree_unflatten(bdims, spec)


def unwrap_batched_leaf(maybe_tensor, vmap_level):
    if isinstance(maybe_tensor, torch.Tensor):
        tensor, bdim = torch._C._functorch._unwrap_batched(maybe_tensor, vmap_level)
        return tensor, bdim
