...
def speedup_experiment(args, model_iter_fn, model, example_inputs, **kwargs):
    ...
    # integration.measure_time_variance()
    ...
    if args.only:
        ...
        for device in args.devices:
            ...
            try:
                ...
                override_model = os.environ.get("override_model", None)
                if override_model == "linear":
                    from test_model import LinearModel
                    model = LinearModel()
                    example_inputs = model.get_example_inputs()
                elif override_model:
                    raise RuntimeError(f"Unexpected override model {override_model}")

                if args.trace_on_xla:
                    import torch_xla.core.xla_model as xm
                    xla_dev = xm.xla_device()
                    model = model.to(device=xla_dev)
                    example_inputs = tree_map(lambda x: x.to(device=xla_dev), example_inputs)
