...
def aot_dispatch_autograd(flat_fn, flat_args: List[Tensor], aot_config: AOTConfig):
    ...
    if config.use_functionalize:
        ...
        fx_g.recompile()
        print("FX_G BUFFERS", [(name, type(t)) for name, t in fx_g.named_buffers()])
    else:
    ...
    with torch.no_grad():
        with track_graph_compiling("joint"):
            ...
            print("bw_module compile time BUFFERS", [type(t) for t in bw_module.buffers()])
    ...
    print("joint_inputs types", [type(j) for j in joint_inputs])
    ...
    class CompiledFunction(torch.autograd.Function):
        ...
        def forward(ctx, *deduped_flat_tensor_args):
            ...
            print("fw_out types", [type(j) for j in fw_outs[0:num_forward_returns]])
            return tuple(fw_outs[0:num_forward_returns])

        @staticmethod
        @disable_torchdynamo
        def backward(ctx, *all_flat_args):
            print("backward in types", [type(j) for j in all_flat_args])
            ...
            print("all_args", [type(j) for j in all_args])
            del contiguous_args
            if CompiledFunction.compiled_bw is None:
                # TODO - pass in fake tensors ?
                context = disable_autocast_manager if disable_amp else nullcontext
                with context(), track_graph_compiling("backward", True):
                    print("BW module parameters", [type(t) for t in bw_module.parameters()])
                    print("BW module buffers", [type(t) for t in bw_module.buffers()])
            ...
            print("backward in outs", [type(j) for j in out])
            return tuple(out)
