...
AT_ERROR("svd: MAGMA library not found in "
  ...
  const auto U_data = U.data_ptr<scalar_t>();
  const auto S_data = S.data_ptr<value_t>();
  const auto Vh_data = Vh.data_ptr<scalar_t>();
  ...
  magma_int_t lwork = -1;
  {
    ...
    magmaSvd<scalar_t, value_t>(jobz, m, n, A_data, lda, S_data, U_data, ldu, Vh_data, ldvh, &wkopt, lwork, rwork, iwork, info_data);
  ...
  for (int64_t i = 0; i < batchsize; i++) {
    ...
    magmaSvd<scalar_t, value_t>(jobz, m, n, A_working_ptr, lda,
                                S_working_ptr, U_working_ptr, ldu, Vh_working_ptr, ldvh, work, lwork, rwork, iwork, info_working_ptr);
  }
#endif
}
...
void svd_magma(const Tensor& A,
               ...
               const Tensor& Vh,
               const Tensor& info) {
  // A, U, S, Vh, info are the right size and strides, but are on GPU
  ...
  const auto A_ = empty_like_cpu(A);
  A_.copy_(A);
  auto U_ = empty_like_cpu(U);
  auto S_ = empty_like_cpu(S);
  auto Vh_ = empty_like_cpu(Vh);
