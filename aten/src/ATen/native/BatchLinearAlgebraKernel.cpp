...
void svd_kernel(const Tensor&,
                ...
                Tensor& Vh,
                const Tensor& infos) {
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(A_copy.scalar_type(), "linalg_svd_cpu", [&]{
    apply_svd<scalar_t>(A_copy, full_matrices, compute_uv, U, S, Vh, infos);
  });
}
