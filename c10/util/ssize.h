...
constexpr auto ssize(const C& c) -> std::
  ...
  using R = std::
      common_type_t<std::ptrdiff_t, std::make_signed_t<decltype(c.size())>>;
  // We use an assert here because we expect this to be exceedingly
  // rare to fire and don't wish to pay a performance hit in release
  // mode.
  assert(!greater_than_max<R>(c.size()));
