...
TensorImpl::TensorImpl(
  ...
  new(mutable_shadow_storage_address()) intrusive_ptr<impl::cow::ShadowStorage>(std::move(shadow_storage));
...
intrusive_ptr<TensorImpl> TensorImpl::take_view() const {
  if (has_shadow_storage_) {
    void* allocation = std::malloc(sizeof(TensorImpl)
                                   + sizeof(intrusive_ptr<impl::cow::ShadowStorage>));
    new(allocation) TensorImpl(*this, shadow_storage_ref());
    return intrusive_ptr<TensorImpl>::unsafe_steal_from_new(reinterpret_cast<TensorImpl*>(allocation));
  } else {
...
const c10::intrusive_ptr<impl::cow::ShadowStorage>& TensorImpl::shadow_storage_ref() const {
  TORCH_INTERNAL_ASSERT(has_shadow_storage_);
  auto& ret = *reinterpret_cast<const c10::intrusive_ptr<impl::cow::ShadowStorage>*>(shadow_storage_address());
...
c10::intrusive_ptr<impl::cow::ShadowStorage>& TensorImpl::shadow_storage_ref() {
  TORCH_INTERNAL_ASSERT(has_shadow_storage_);
  auto& ret = *reinterpret_cast<c10::intrusive_ptr<impl::cow::ShadowStorage>*>(mutable_shadow_storage_address());
  TORCH_INTERNAL_ASSERT(ret != nullptr);
  return ret;
}
