...
auto C10_API make_copy_on_write(Storage const& storage)
  ...
  std::unique_ptr<void, DeleterFnPtr> original_ctx_rep =
      data_ptr.move_context();
  assert(original_ctx_rep.get() == data_ptr.get());
  detail::UniqueVoidPtr original_ctx(
      original_ctx_rep.get(),
      original_ctx_rep.get(),
      original_ctx_rep.get_deleter());
...
CopyOnWriteContext::CopyOnWriteContext(detail::UniqueVoidPtr ctx)
    : original_ctx_(std::move(ctx)) {
  assert(
      original_ctx_.get_deleter() !=
