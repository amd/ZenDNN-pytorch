...
class CopyOnWriteContext {
 public:
  ...
  explicit CopyOnWriteContext(detail::UniqueVoidPtr ctx);
 ...
 private:
  ...
  detail::UniqueVoidPtr original_ctx_;
...
auto make_data_ptr(at::DataPtr const& data_ptr, CopyOnWriteContext& ctx) -> at::DataPtr {
  ctx.increment_refcount();
  return at::DataPtr(data_ptr.get(), &ctx, CopyOnWriteContext::delete_instance, data_ptr.device());
}

} // namespace

auto C10_API make_copy_on_write(Storage const& storage) -> c10::intrusive_ptr<StorageImpl> {
  ...
  if (data_ptr.get_deleter() == CopyOnWriteContext::delete_instance) {
    ...
    CopyOnWriteContext& ctx = *data_ptr.cast_context<CopyOnWriteContext>(CopyOnWriteContext::delete_instance);
    return c10::make_intrusive<StorageImpl>(StorageImpl::use_byte_size_t(), storage_impl.sym_nbytes(),
                                            make_data_ptr(data_ptr, ctx),
                                            storage_impl.allocator(), storage_impl.resizable());
  }
  ...
  std::unique_ptr<void, DeleterFnPtr> original_ctx_rep = data_ptr.move_context();
  assert(original_ctx_rep.get() == data_ptr.get());
  detail::UniqueVoidPtr original_ctx(original_ctx_rep.get(), original_ctx_rep.get(), original_ctx_rep.get_deleter());
  original_ctx_rep.release(); // already moved into original_ctx

  std::unique_ptr<CopyOnWriteContext, AssertFalseDeleter> ctx(new CopyOnWriteContext(std::move(original_ctx)));
  storage_impl.set_data_ptr_noswap(make_data_ptr(data_ptr, *ctx));

  return c10::make_intrusive<StorageImpl>(StorageImpl::use_byte_size_t(), storage_impl.sym_nbytes(),
                                          make_data_ptr(data_ptr, *ctx.release()), storage_impl.allocator(),
                                          storage_impl.resizable());
}

auto copy_on_write_refcount(Storage const& storage) -> std::optional<std::int64_t> {
...
CopyOnWriteContext::CopyOnWriteContext(detail::UniqueVoidPtr ctx) : original_ctx_(std::move(ctx)) {
  assert(original_ctx_.get_deleter() != delete_instance); // we never wrap a CopyOnWriteContext
}
