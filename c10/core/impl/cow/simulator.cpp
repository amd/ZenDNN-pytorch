...
SimulatorImpl<intrusive>::SimulatorImpl(std::uint64_t storage_generation) noexcept
...
template <bool intrusive>
auto SimulatorImpl<intrusive>::bump_storage_generation() noexcept -> std::uint64_t {
  std::lock_guard<std::mutex> lock(mtx_);
