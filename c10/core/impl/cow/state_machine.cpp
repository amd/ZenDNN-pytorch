...
auto cow::StateMachine::Impl::maybe_bump(
  ...
  TORCH_INTERNAL_ASSERT(physical_generation_ != std::numeric_limits<Generation>::max());
  Generation physical_generation = ++physical_generation_;
  apply_to_active_shadow_storage(
      maybe_shadow_storage, default_shadow_storage_, [&physical_generation](auto& shadow_storage) {
        shadow_storage.update_from_physical_generation(physical_generation);
      });
}
