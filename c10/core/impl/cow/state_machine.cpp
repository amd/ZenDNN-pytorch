...
auto cow::StateMachine::maybe_bump(cow::ShadowStorage* maybe_shadow_storage)
  ...
  std::uint64_t shadow_generation =
      shadow_generation_with_lock(maybe_shadow_storage);
...
auto cow::StateMachine::shadow_generation_with_lock(
    cow::ShadowStorage* maybe_shadow_storage) -> std::uint64_t {
  TORCH_INTERNAL_ASSERT(default_shadow_storage_.has_value());
  return maybe_shadow_storage != nullptr
