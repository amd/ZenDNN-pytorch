...
auto State::maybe_bump(cow::Simulator* maybe_simulator) -> void {
...
auto State::storage_generation() -> std::uint64_t {
  std::lock_guard<std::mutex> lock(mtx_);
  return physical_generation_;
}

auto State::simulate_copy_on_write(cow::Simulator* simulator) -> intrusive_ptr<cow::Simulator> {
  std::lock_guard<std::mutex> lock(mtx_);
