...
class C10_API StateMachine {
 ...
 private:
  // Gets the shadow generation while the mutex is already held.
  auto shadow_generation_with_lock(cow::ShadowStorage* maybe_shadow_storage)
      -> std::uint64_t;
  ...
  std::atomic<StateId> state_id_ = StateId::initial;
