...
auto CopyOnWriteState::maybe_bump(CopyOnWriteSimulator* maybe_simulator)
  ...
  std::uint32_t physical_generation = ++physical_generation_;
  std::uint32_t shadow_generation = simulator.bump_storage_generation();
...
auto CopyOnWriteState::storage_generation() -> std::uint32_t {
  std::lock_guard lock(mtx_);
...
auto CopyOnWriteState::simulate_copy_on_write(CopyOnWriteSimulator* simulator)
  ...
  if (default_simulator_ == nullptr) {
    ...
    default_simulator_ =
        make_intrusive<CopyOnWriteSimulator>(physical_generation_);
  }
