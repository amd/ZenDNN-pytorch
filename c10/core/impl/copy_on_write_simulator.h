...
class CopyOnWriteSimulator final : private intrusive_ptr_target {
 public:
  ...
  // Creates a new copy of this simulator. This is the constructor you
  // should invoke if you are making a logical copy.
  CopyOnWriteSimulator(CopyOnWriteSimulator const& that) noexcept = default;

  // Gets the current generation.
  auto storage_generation() const noexcept -> std::uint32_t {
    return storage_generation_;
  }

  // Increments the current generation.
  auto bump_storage_generation() noexcept -> void {
    assert(storage_generation_ != std::numeric_limits<std::uint32_t>::max());
    ++storage_generation_;
  }

 private:
  // TODO Decide on atomicity of accesses to the generation. From the
  // user's perspective, copies are fully distinct and require no
  // external synchronization, so we need to ensure this field's
