...
TEST_F(CopyOnWriteTest, Simulator) {
  CopyOnWriteSimulator simulator(/*storage_generation=*/0);
  ASSERT_THAT(simulator.storage_generation(), testing::Eq(0));
  ASSERT_THAT(simulator.bump_storage_generation(), testing::Eq(1));
}

TEST_F(CopyOnWriteTest, StateWithoutSimulator) {
  CopyOnWriteState state;
  state.maybe_bump(nullptr);
  ASSERT_THAT(state.storage_generation(), testing::Eq(0));
}
...
TEST_F(CopyOnWriteTest, MultipleViewFamilies) {
  CopyOnWriteState state;
  intrusive_ptr<CopyOnWriteSimulator> family_2 =
      state.simulate_copy_on_write(nullptr);
  ...
  state.maybe_bump(/*simulator=*/nullptr);
