...
class TestMKLDNNVerbose(TestCase):
    def test_verbose_on(self):
        ...
        with subprocess.Popen('python -u {}/mkldnn_verbose.py --verbose-level=1'.format(loc), shell=True,
    ...
    def test_verbose_off(self):
        num = 0
        loc = os.path.dirname(os.path.abspath(__file__))
        with subprocess.Popen('python -u {}/mkldnn_verbose.py --verbose-level=0'.format(loc), shell=True,
                              stdout=subprocess.PIPE, stderr=subprocess.STDOUT) as p:
            for line in p.stdout.readlines():
                line = str(line, 'utf-8').strip()
                if line.startswith("onednn_verbose"):
