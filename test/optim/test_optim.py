...
class OptimizerInfo:
    ...
    def __init__(self,
        ...
        self.optim_kwargs_func = optim_kwargs_func
...
# ----------------------------------------------------------------------------------------------------------------
# NOTE: The following optim_kwargs_* sampling functions only return constructor combinations of NON-IMPLEMENTATION
# -CHANGING flags, i.e., flags that are not foreach, fused, or differentiable.

def optim_kwargs_adadelta(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    return [
        Adadelta(params),
        Adadelta(params, weight_decay=0.5),
        Adadelta(params, weight_decay=0.5, maximize=True),
        Adadelta(params, rho=0.95, weight_decay=0.5),
    ]

def optim_kwargs_adagrad(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # We only return constructor combinations of NON-IMPLEMENTATION-CHANGING flags, i.e., flags that are not
    # foreach, fused, or differentiable.
    return [
        Adadelta(params),
        Adadelta(params, weight_decay=0.5),
        Adadelta(params, weight_decay=0.5, maximize=True),
        Adadelta(params, initial_accumulator_value=0.1, weight_decay=0.5),
        Adadelta(params, lr=0.1, lr_decay=0.1, weight_decay=0.5),
    ]


def optim_kwargs_adam(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # We only return constructor combinations of NON-IMPLEMENTATION-CHANGING flags, i.e., flags that are not
    # foreach, fused, or differentiable.
    return [
        Adadelta(params),
...
def optim_kwargs_adamax(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # We only return constructor combinations of NON-IMPLEMENTATION-CHANGING flags, i.e., flags that are not
    # foreach, fused, or differentiable.
    return [
        Adadelta(params),
...
def optim_kwargs_adamw(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # We only return constructor combinations of NON-IMPLEMENTATION-CHANGING flags, i.e., flags that are not
    # foreach, fused, or differentiable.
    return [
        Adadelta(params),
...
def optim_kwargs_asgd(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # We only return constructor combinations of NON-IMPLEMENTATION-CHANGING flags, i.e., flags that are not
    # foreach, fused, or differentiable.
    return [
        Adadelta(params),
...
def optim_kwargs_lbfgs(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # may have to think about params. Either here we return partials, or we pass in the params to here.
    return []


def optim_kwargs_nadam(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # may have to think about params. Either here we return partials, or we pass in the params to here.
    return []


def optim_kwargs_radam(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # may have to think about params. Either here we return partials, or we pass in the params to here.
    return []


def optim_kwargs_rmsprop(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # may have to think about params. Either here we return partials, or we pass in the params to here.
    return []


def optim_kwargs_rprop(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # may have to think about params. Either here we return partials, or we pass in the params to here.
    return []


def optim_kwargs_sgd(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # may have to think about params. Either here we return partials, or we pass in the params to here.
    return []


def optim_kwargs_sparseadam(optim_info: OptimizerInfo, params: Union[List[Parameter], List[Tensor], Dict[Any]]):
    # may have to think about params. Either here we return partials, or we pass in the params to here.
    return []
...
module_db: List[OptimizerInfo] = [
    OptimizerInfo(Adadelta,
                  optim_kwargs_func=optim_kwargs_adadelta,
                  supported_impl_kwargs=['foreach', 'differentiable'],
    ),
    OptimizerInfo(Adagrad,
                  optim_kwargs_func=optim_kwargs_adagrad,
                  supported_impl_kwargs=['foreach', 'differentiable'],
                  supports_sparse_on=['cpu'],
    ),
    OptimizerInfo(Adam,
                  optim_kwargs_func=optim_kwargs_adam,
                  supported_impl_kwargs=['foreach', 'differentiable', 'fused'],
    ),
    OptimizerInfo(Adamax,
                  optim_kwargs_func=optim_kwargs_adamax,
                  supported_impl_kwargs=['foreach', 'differentiable'],
    ),
    OptimizerInfo(AdamW,
                  optim_kwargs_func=optim_kwargs_adamw,
                  supported_impl_kwargs=['foreach', 'differentiable', 'fused'],
    ),
    OptimizerInfo(ASGD,
                  optim_kwargs_func=optim_kwargs_asgd,
                  supported_impl_kwargs=['foreach', 'differentiable'],
    ),
    OptimizerInfo(LBFGS,
                  optim_kwargs_func=optim_kwargs_lbfgs,
                  supported_impl_kwargs=[],
    ),
    OptimizerInfo(NAdam,
                  optim_kwargs_func=optim_kwargs_nadam,
                  supported_impl_kwargs=['foreach', 'differentiable'],
    ),
    OptimizerInfo(RAdam,
                  optim_kwargs_func=optim_kwargs_radam,
                  supported_impl_kwargs=['foreach', 'differentiable'],
    ),
    OptimizerInfo(RMSprop,
                  optim_kwargs_func=optim_kwargs_rmsprop,
                  supported_impl_kwargs=['foreach', 'differentiable'],
    ),
    OptimizerInfo(Rprop,
                  optim_kwargs_func=optim_kwargs_rprop,
                  supported_impl_kwargs=['foreach', 'differentiable'],
    ),
    OptimizerInfo(SGD,
                  optim_kwargs_func=optim_kwargs_sgd,
                  supported_impl_kwargs=['foreach', 'differentiable'],
